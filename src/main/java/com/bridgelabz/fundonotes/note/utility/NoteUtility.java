...
public class NoteUtility {
	...
	public static void validateNoteCreation(CreateDTO createDto) throws NoteCreationException, NullValueException {
		...
		for(int i=0;i<createDto.getLabels().size();i++) {
			if(createDto.getLabels().get(i).getLabelName().equals(null)||createDto.getLabels().get(i).getLabelName().length()==0||createDto.getLabels().get(i).getLabelName().trim().length()==0) {
